...
6. Fix the accepted findings, red-first where testable. For each one:
   ...
   **THE DRIVE GATE RE-ARMS** (afk-playbook-bridge, ticket r773cwzq): a
   disposition fix that touches DRIVEN SURFACE — code the verify-by-driving
   evidence exercised — INVALIDATES that evidence, and before this item's
   run closes (step 8's sign-off included) the changed flow is re-driven.
   The authoritative statement of the rule — what re-arms the gate, and the
   one narrow behavior-unchanged waiver — is task-lifecycle.md's
   "Drive evidence has a freshness rule" paragraph; this loop adds only the
   journaling shape:

       nahel log note --item <item-id> --run <run-id> \
         --data summary="drive evidence re-armed by fix <finding-event-id>: <re-drove: steps and observations | behavior-unchanged waiver: the equivalence argument>" \
         --data drive=<the prior drive-evidence event id>

   Silent reliance on a pre-fix drive is how the murder-mystery dogfood
   shipped its narration stutter: fixes landed during review AFTER the live
   drive, nobody re-drove, and the stored evidence vouched for bytes that no
   longer ran.
