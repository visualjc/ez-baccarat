...
3. Create the children, each after the items it depends on:
       ...
       nahel item new <type> <slug> <lane> --parent <feature-id> [--depends-on <id>]... [--playbook <name>]
   ...
   **Playbook copy** (afk-playbook-bridge, decisions vy4vn7de + j93bhc15):
   when the parent names a `playbook`, COPY it onto each child you expect a
   drive to work — pass `--playbook <the parent's value>` explicitly. When
   the parent is BARE but the store carries a build recipe (`nahel playbook
   list`), DEFAULT that recipe onto the implementable leaves anyway — the
   murder-mystery evidence is that one build recipe fits every implementable
   leaf, and ceremony is modulated inside the drive by journaled role-skips,
   not by leaving items bare. `qa` and `plan` children stay bare until a
   recipe earns them. A child left bare on purpose, or given a different
   recipe, is a choice step 5's note states. There is no inheritance at read
   time: a drive reads the field off the item it targets and nowhere else,
   so a child you leave bare is a child the AFK wave bare-dispatches.
