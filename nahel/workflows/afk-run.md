...
8. Dispatch the worker. Run the checkpoint check (step 3) first.

   **THE PLAYBOOK FORK** (afk-playbook-bridge, decision fxb9jqba) — before
   any bare dispatch, read the item's record: an item that names a `playbook`
   is driven through `nahel/workflows/playbook-run.md` for that playbook, and
   the drive IS this dispatch step — its steps' dispatches replace the single
   bare one below. The item rejoins this workflow at step 9 only after
   playbook-run's own close (its section 8: last step committed, the drive's
   run ENDED with its honest outcome, the item flipped to `in-review`) —
   never at the last step-commit alone, which would leave an active run and
   an in-progress item under a step that assumes both are settled. Only an
   item naming no playbook falls through to the bare dispatch.

   - Bare-dispatching an item that names a playbook is a DEVIATION: legal
     only with a journaled reason (`nahel log note --item <id>`) stating why
     the recipe was set aside — the defaults-with-teeth rule, verbatim.
   - A step that cannot commit mid-drive — after playbook-run's own per-role
     honesty ladder (honest envelope, one retry, tiered self-fallback, its
     section 5) — PARKS the item (step 12): status `blocked`, with a note
     naming the playbook, the step id, and the drive run. NEVER a silent
     fall-back to bare dispatch: the recipe's evidence trail is the reason
     the playbook exists, and an AFK wave that quietly ships around it has
     shipped un-reviewed work.
   - A playbook that fails to RESOLVE at drive start is the same park, and
     `nahel validate` warned about it before the wave began
     (item.playbook-unresolvable) — an AFK run that starts on a store with
     that warning outstanding has chosen to hit it live.

   For the bare dispatch: write the brief — the task, stated so a fresh agent
   can execute it — to a file, and dispatch a pointer to it:
