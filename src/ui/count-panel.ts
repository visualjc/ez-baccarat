...
export function mountCountPanel(host: HTMLElement, bus: GameBus): CountPanelHandle {
  ...
  const list = make("ol", "trace-list"); list.setAttribute("role", "log"); list.setAttribute("aria-live", "off"); list.tabIndex = 20; const foot = make("p", "trace-foot", "decks unseen = (416 − seen) ÷ 52 · face-down burns count as unseen"); trace.append(traceHead, columns, list, foot);
  const summary = make("section", "delta-summary"); summary.setAttribute("aria-live", "polite");
  const rules = make("details", "rules-card"); const rulesSummary = make("summary", undefined, "Count rules & thresholds"); rulesSummary.tabIndex = 19; rulesSummary.append(make("span", "chev", "▸")); rules.append(rulesSummary);
